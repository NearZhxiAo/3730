    ...
    switch (cmd) {
      case SDMA_IOCREQUESTDMA:
        ...
        if (result) {
            ...
            channels[channel].owner = current;
      ...
      case SDMA_IOCRELEASEDMA:
        ...
        /* should probably check that channels[channel].owner == current */

        /* NULL owner field only since ISR operates based on owner != NULL */
        channels[channel].owner = NULL;

        /* do we need to "undo" the refcount++ of get_user_pages()? */
        page_cache_release(channels[channel].ptransferStatePage);

        kunmap(channels[channel].ptransferStatePage);
      ...
      case SDMA_IOCWAITFORCOMPLETION:
        ...
        if (channels[channel].owner == current) {
...
/*
 *  @(#) ti.sdo.linuxutils.sdma; 1, 0, 0,70; 7-16-2009 17:09:03; /db/atree/library/trees/linuxutils/linuxutils-e04x/src/
 */
