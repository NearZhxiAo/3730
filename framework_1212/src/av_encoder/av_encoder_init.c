...
/*********************************************************************/
	...
	phandle->inbuffer  =CMEM_alloc(CMEM_SIZE(phandle->width, phandle->height), &phandle->cmem_params);
	phandle->outbuffer = CMEM_alloc(CMEM_SIZE(phandle->width, phandle->height), &phandle->cmem_params);

	if(phandle->inbuffer == NULL || phandle->outbuffer == NULL)
	{
	...
	else
	{
		...
		SYS_INFO("inbuffer virtaddr is %x phys addr is %x.\r\n", phandle->inbuffer,CMEM_getPhys(phandle->inbuffer));
		SYS_INFO("outbuffer virtaddr is %x phys_addr is %x.\r\n", phandle->outbuffer,CMEM_getPhys(phandle->outbuffer));
	}

	//h264  init
	H264_INIT(phandle->inbuffer, phandle->outbuffer, phandle->width, phandle->height, phandle->framerate ,phandle->bitrate ,phandle->intra);

	JPEG_INIT(phandle->inbuffer, phandle->outbuffer, phandle->width, phandle->height, phandle->qvalue);
