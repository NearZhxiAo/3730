...
static void xml_append(request *req, const char *fmt, ...)
{
	...
	len = strlen(text);
	//fprintf(stderr,"\r\n Before xml_append if condition with len: %d,xml length: %d,xml size: %d\n",len,req->xmllen,req->xmlsize);
	...
	memcpy(buf, text, len);
	//fprintf(stderr,"\r\n After memcpy, buf: %p, text: %s, length: %d\n",buf,text,len);
	//fprintf(stderr,"\r\n Buffer Content: \n");
	//for(i=0; i <= len; i++)
...
{
	//fprintf(stderr,"\r\n Inside xadd_elem with tag : %s val : %s\n",tag,val);
