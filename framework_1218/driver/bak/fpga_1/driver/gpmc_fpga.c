...
void gpmc_cs_write_reg(int cs, int idx, u32 val)
{
	...
	reg_add = gpmc_base + GPMC_CS0 + (cs * GPMC_CS_SIZE) + idx;

	__raw_writel(val, reg_add);
}
...
static int __init fpga_init_cdev(void)
{
	...
	printk("Getting Chip Select\n");
       ...
       /* gpmc_base=ioremap(0x6E000000,SZ_4K);
        ...
        gpmc_base=ioremap(0x6E000000, 4096);
       ...
       //iounmap(gpmc_base);
        ...
        while(j++<5)     
          {  
           ...
           writeb(0xA5,fpga_base+0x380);
           writeb(0xA4,fpga_base+0x381);
           writeb(0xA6,fpga_base+0x382);
           tmp=readb (fpga_base + 0x380);
		   tmp1 = readb (fpga_base + 0x381);
		   tmp2 = readb (fpga_base + 0x382);
		   
           printk("tmp:%4x,  tmp1:%4x, tmp2:%4x\n",tmp, tmp1, tmp2);
           //printk("tmp:%4x\n",tmp);
          }
	return 0;
}
