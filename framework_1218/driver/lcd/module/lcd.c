...
void printk_VPBEREG(void)
{
	volatile unsigned int *pReg = NULL;
	unsigned int i = 0;
	pReg = (unsigned int *)IO_ADDRESS(0x1C40044); 
	printk("VPSS_CLK_CTRL = 0x%x\r\n", *pReg);
	
	pReg = (unsigned int *)IO_ADDRESS(0x1C40004); 
	*pReg &= (~(1<<22));
	printk("PINMUX1 = 0x%x.\r\n", *pReg);
	pReg = (unsigned int *)IO_ADDRESS(0x1c70200); 
	printk("VPBE_CLK_CTRL = 0x%x.\r\n", *pReg);
	for (i = 0; i < 90 ;i++)
	{
		pReg = (unsigned int *)IO_ADDRESS(0x1c71E00+i*4); 
		printk("vpbe: 0x%x value 0x%x.\r\n", i*4 , *(pReg));
	}

	pReg = (unsigned int *)IO_ADDRESS(0x1c71C00+0x04); 

	*pReg = 0x101;

	printk("\r\n\r\n");
	printk("vpbe OSD: 0x4 value 0x%x.\r\n",  *(pReg));
	printk("\r\n\r\n");

	for (i = 0; i < 64 ;i++)
	{
		pReg = (unsigned int *)IO_ADDRESS(0x1c71C00+i*4); 
		printk("vpbe OSD: 0x%x value 0x%x.\r\n", i*4 , *(pReg));
	}
