...
static LIBASSHM_OBJ *libasshm_init()
{
	...
	mmap_offset = sizeof(pthread_mutex_t) + 3 * sizeof(int);
	...
	{ 
		...
		if(0 != pthread_mutex_init( &pmt ,NULL))
		{
		...
		*(int*)(phandle->asshm_mmap + sizeof(pthread_mutex_t) + sizeof(int) + sizeof(int)) = 0x00;
		//pthread_mutex_unlock(&pmt);
	}
...
#if 0
static succeed_type libasshm_get_seq(LIBASSHM_OBJ *phandle, int *ptr_seq)
{
	...
	{
		int seq = *(int*)(phandle->asshm_mmap + sizeof(pthread_mutex_t) + sizeof(int) + sizeof(int));
		seq++;
		*ptr_seq = seq;
		*(int*)(phandle->asshm_mmap + sizeof(pthread_mutex_t) + sizeof(int) + sizeof(int)) = seq;
	}
	return succeed_type_succeed;
...
static succeed_type libasshm_get_seq(LIBASSHM_OBJ *phandle, int *asshm_seq)
{
	unsigned int mmap_offset = 0;
	unsigned int asshm_addr = 0;
	...
	mmap_offset = sizeof(pthread_mutex_t) + 2 * sizeof(int);
	asshm_addr =  (phandle->asshm_mmap + mmap_offset);
	*asshm_seq = *(int *)asshm_addr;
	SYS_INFO("asshm seq is %d..\r\n", *asshm_seq);
 ...
 *                  succeed_type_failed  -- failed
 * **********************************************/
static succeed_type libasshm_put(LIBASSHM_OBJ *phandle,int asshm_seq,  void *ptr, int length)
{
	unsigned int mmap_offset = 0;
	unsigned int asshm_addr = 0;
	unsigned int asshm_offset = 0;
	...
	*(int *)(asshm_addr+sizeof(sem_t)) = length;
	*(int *)(phandle->asshm_mmap + sizeof(pthread_mutex_t) + 2 * sizeof(int)) = (asshm_seq + 1) % phandle->asshm_size;
	asshm_seq = *(int *)(phandle->asshm_mmap + sizeof(pthread_mutex_t) + 2 * sizeof(int)) ;
	SYS_INFO("sequence is %d..\r\n",asshm_seq);
	//memcpy((void *)(asshm_addr+sizeof(sem_t)+sizeof(LIBASSHM_UINT_HEADER)), ptr,length);
	
	memcpy((void *)(asshm_addr + sizeof(sem_t) + sizeof(int)), ptr, length);
	//pthread_mutex_unlock(pmutex);
...
succeed_type ASSHM_GET_SEQ(LIBASSHM_OBJ *phandle, int *ptr_seq)
{
	return libasshm_get_seq(phandle, ptr_seq);
}
succeed_type ASSHM_PUT(LIBASSHM_OBJ *phandle, int asshm_seq, void *ptr, int length)
{
	return libasshm_put(phandle, asshm_seq,  ptr, length);
}
