...
int boa(int argc, char **argv)
{
  ...
  //  while ((c = getopt(argc, argv, "c:r:d")) != -1) {
    ...
    //printf("InitFileMsgDrv ok!\n");

    if (hash_table_init() < 0) {
	printf("init hash_table_init failed!\n");
	exit(1);
    }

    if(arg_hash_table_init() < 0) {
	printf("init arg_hash_table_init failed!\n");
	exit(1);
    }

    if(uri_hash_table_init() < 0) {
	printf("init uri_hash_table_init failed!\n");
	exit(1);
    }
#ifdef PSIA_MEDIA
    ...
    fixup_server_root();
    //printf("fixup_server_root ok!\n");
    read_config_files();
    
    //printf("read_config_files ok\n");

#ifndef ISLAB 	//2010-08-21
    ...
    //printf("init_signals ok!\n");

    drop_privs();
    //printf("drop_privs ok!\n");

    create_common_env();
    //printf("create_common_env ok!\n");

    build_needs_escape();
    //printf("build_needs_escape ok!\n");
...
static int create_server_socket(void)
{
	...
	/* reuse socket addr allow the bindip use*/
...
#ifdef ISLAB_DEBUG
    printf("entering fixup_server_root....!\n");
#endif
