...
#define STORAGE_FUNCTION_NAME "GenerateFileName_ByCurTime"
    ...
    sprintf(pstFileName, "%04d_%02d_%02d_%02dh_%02dm_%02ds_%03ldms.%s",
...
long long GetDiskfreeSpace(char* pDisk)
{
	...
	{
		//	printf("%ld  %ld",disk_statfs.f_bsize,disk_statfs.f_bfree);
		//freespace = (((long long)disk_statfs.f_bsize * (long long)disk_statfs.f_bfree)>>30);
...
int main()
{
	...
	printf("cCurpwd is %s\r\n", cCurPwd);
	CreateIdxDirMulti(DATA_DIR_JPEG_NAME_ROOT);
	CreateIdxDirMulti(DATA_DIR_VIDEO_NAME_ROOT);

	//snprintf(jpgdir, 100, "%s/pic1", DATA_DIR_JPEG_NAME_ROOT);
	...
	SYS_INFO("file name is %s\r\n", acDataDirName);

	DiskfreeSpace = GetDiskfreeSpace("/opt/sdb");
	SYS_INFO("free space is %d\r\n", DiskfreeSpace);
